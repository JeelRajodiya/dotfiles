...
function estimatedUsageCost(
	...
	const rates = [...(model.cost.tiers ?? [])]
		...
		.find((tier) => inputTokens > tier.inputTokensAbove) ?? model.cost;
...
function computeUsageTotals(
	...
	for (const entry of entries) {
		...
		cost = addUsageTotal(cost, usage.cost || estimatedUsageCost(ctx, entry, usage));
...
export function getUsageTotals(ctx: ExtensionContext): UsageTotals {
	...
	const totals = computeUsageTotals(entries, ctx);
...
export function buildContextDisplayLabel(options: {
	...
	const { percent, tokens, contextWindow, style = "text", asciiGauge = false } = options;
	if (!contextWindow || contextWindow <= 0) return "--";

	const used = Number.isFinite(tokens)
		? Math.max(0, tokens ?? 0)
		: Math.max(0, ((percent ?? 0) / 100) * contextWindow);
	const usedLabel = formatCount(Math.round(used)).replace(".0k", "k");
	const text = `${usedLabel}/${formatCount(contextWindow)}`;
