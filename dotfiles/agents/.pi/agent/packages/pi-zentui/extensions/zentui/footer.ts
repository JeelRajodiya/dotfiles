		...
		return {
			...
			render(width: number): string[] {
				...
				const contextLabel = buildContextDisplayLabel({
					...
					tokens: useLiveContext ? liveContext.tokens : contextUsage?.tokens,
					contextWindow,
				...
				const footerRightExtensions = usesCustomFormat ? extensionRightSegments : [];
				...
				const fullZones = {
					...
					right: prependStatusArea(
						contentRight,
						joinStatusTexts(footerRightExtensions, separator),
						separator,
					),
				};
